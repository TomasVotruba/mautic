...
class LeadSubscriberFunctionalTest extends MauticMysqlTestCase
{
    ...
    {
        $searchViewStats  = $focusModel->getStatRepository()->getStatsViewByLead((int) $lead->getId(), [
            'search'=>$phrase,
        ]);
        $searchClickStats = $focusModel->getStatRepository()->getStatsClickByLead((int) $lead->getId(), [
            'search'=>$phrase,
        ]);
