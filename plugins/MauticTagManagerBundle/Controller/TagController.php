...
class TagController extends FormController
{
    ...
    public function indexAction(Request $request, $page = 1): Response
    {
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $returnUrl = $this->generateUrl('mautic_tagmanager_index', [
                'page' => $lastPage,
            ]);
        ...
        return $this->delegateView([
            ...
            'passthroughVars' => [
                'activeLink'    => '#mautic_tagmanager_index',
                'route'         => $this->generateUrl('mautic_tagmanager_index', [
                    'page' => $page,
                ]),
                'mauticContent' => 'tags',
            ],
    ...
    public function newAction(Request $request, TagDependencies $tagDependencies)
    {
        ...
        if (null === $response) {
            $response = $this->delegateView([
                ...
                'passthroughVars' => [
                    'activeLink'    => '#mautic_tagmanager_index',
                    'route'         => $this->generateUrl('mautic_tagmanager_action', [
                        'objectAction' => 'new',
                    ]),
                    'mauticContent' => 'tagmanager',
                ],
    ...
    public function editAction(Request $request, TagDependencies $tagDependencies, int $objectId, bool $ignorePost = false): Response
    {
        ...
        try {
            return $this->createTagModifyResponse(
                ...
                $tagDependencies,
                $postActionVars,
                $this->generateUrl('mautic_tagmanager_action', [
                    'objectAction' => 'edit',
                    'objectId' => $objectId,
                ]),
                $ignorePost
        ...
        if (!$this->isFormCancelled($form)) {
            if ($this->isFormValid($form)) {
                ...
                if ($this->getFormButton($form, ['buttons', 'apply'])->isClicked()) {
                    ...
                    $editAction = $this->generateUrl('mautic_tagmanager_action', [
                        'objectAction' => 'edit',
                        'objectId' => $tag->getId(),
                    ]);
    ...
    private function getPostActionVars(Request $request, ?int $objectId = null): array
    {
        // set the return URL
        if ($objectId) {
            $returnUrl       = $this->generateUrl('mautic_tagmanager_action', [
                'objectAction' => 'view',
                'objectId'=> $objectId,
            ]);
            $viewParameters  = [
