...
class OneSignalApiTest extends TestCase
{
    public function testAddMobileData(): void
    {
        ...
        $data = [];
        $method->invokeArgs($mockOneSignalApi, [
            &$data, [
                'ios_subtitle' => 'test',
            ]]);
        $this->assertEquals([
        ...
        $method->invokeArgs($mockOneSignalApi, [
            &$data, [
                'android_led_color' => 'test',
            ]]);
        $this->assertEquals([
            'android_led_color' => 'FFTEST',
        ], $data);

        $data = [];
        $method->invokeArgs($mockOneSignalApi, [
            &$data, [
                'android_accent_color' => 'test',
            ]]);
        $this->assertEquals([
            'android_accent_color' => 'FFTEST',
        ], $data);

        $data = [];
        $method->invokeArgs($mockOneSignalApi, [
            &$data, [
                'android_group_key' => 'test',
            ]]);
        $this->assertEquals([
            'android_group' => 'test',
        ], $data);
