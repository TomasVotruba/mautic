...
final class BuilderSubscriberTest extends TestCase
{
    ...
    {
        $this->emailModel->expects($this->atLeastOnce())->method('buildUrl')->willReturn('https://some.url');
        $this->translator->method('trans')->willReturn('some translation');
        $this->coreParametersHelper->expects($this->atLeastOnce())->method('get')->willReturnCallback(function ($key): string|false {
            if ('locale' === $key) {
    ...
    public function testUnsubscribeTestTokensAreReplacedWithHashOnEmailGenerate(): void
    {
        ...
        $this->emailModel->expects($this->atLeastOnce())->method('buildUrl')
